...
export function BigStepper({
  ...
  const [slideDir, setSlideDir] = useState<1 | -1>(1); // 1 = up (increment), -1 = down (decrement)
  ...
  // Slide distance in px — new value comes from below on increment, above on decrement
  const yIn  = slideDir === 1 ? 32 : -32;
  const yOut = slideDir === 1 ? -32 : 32;

  return (
    <div className="flex flex-col items-center gap-4 w-full">

      {/* Número com slide direcional */}
      <div className="w-full flex flex-col items-center" style={{ minHeight: "6.5rem", overflow: "hidden" }}>
        <AnimatePresence mode="popLayout" initial={false}>
          ...
          >
            <input
              ...
              className={`w-full text-center text-7xl font-black bg-transparent border-none outline-none leading-none ${
        ...
        {unit && (
          <span className="text-xl text-muted-foreground font-semibold tracking-wide mt-1">
            {unit}
          </span>
        )}
      </div>
