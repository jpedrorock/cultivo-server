...
def main():
    ...
    for png_name, sym in ICONS.items():
        ...
        c_parts.append("};")
        c_parts.append("")
        c_parts.append(f"const lv_image_dsc_t {sym} = {{")
        c_parts.append(f"  .header.magic = LV_IMAGE_HEADER_MAGIC,")
        c_parts.append(f"  .header.cf = LV_COLOR_FORMAT_A8,")
        c_parts.append(f"  .header.flags = 0,")
        c_parts.append(f"  .header.w = {w},")
        c_parts.append(f"  .header.h = {h},")
        c_parts.append(f"  .header.stride = {w},")
